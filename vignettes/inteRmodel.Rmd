...
In this use case we want to find the best model to relate the gene expression on a glioma with 
...
The basic workflow proposed is the following:

 1. Select a model for the data
 2. Look for different models that could be better
 3. Change the weight of the model to tune them.
 4. Compare models with the same bootstrap
 
This steps are explained on the detail on the next sections.
...
iterations <- 10
index <- vector("list", length = iterations)
...
points(agg[, 2:3], pch = 17, col = c("red", "green", "black"))
```
